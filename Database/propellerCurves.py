...
fitOfThrustFitOrder = 4 #Order of polynomial fit to thrust coefs vs rpm
fitOfPowerFitOrder = 6 #Order of polynomial fit to power coefs vs rpm

thrustZeroCoefs = [4] #Which polynomial fit coefficients should be set to zero for Selig's props
powerZeroCoefs = [4,5,6]
...
for propFolder in wholeDatabase:
    ...
    if dataType == "apc": #Data files from APC (manufacturer)
        ...
        for i in range(thrustFitOrder+1):
            ...
            #print(r**2)
            rpmSpace = np.linspace(min(rpms), max(rpms), 100)
        ...
        for rpm in rpms:
            ...
            thrust = fit.poly_func(a, advRatioSpace)
            ax.plot(advRatioSpace, rpmTheo+500, thrust, "g-")
        ...
        for rpm in rpms:
            ...
            power = fit.poly_func(a, advRatioSpace)
            ax.plot(advRatioSpace, rpmTheo+500, power, "g-")
        ...
        for rpm in rpms:
            ...
            a = fit.poly_func(fitOfThrustFit.T, rpm+500)
            thrust = fit.poly_func(a, advRatioSpace)
            plt.plot(advRatioSpace, rpmTheo+500, thrust, "g-")
...
print("Successfully analyzed and stored", propCount, "props.")
dbcur.execute("select * from props")
print(dbcur.fetchall())
